...
/*递归找元素
*
* 找一个 属性 attr = value 值 的元素
* */

function findParentEle(ele,attr,value){

    var obj = null;

    findEle(ele, attr, value);
    ...
    function findEle(ele,attr,value){
        ...
        if(ele.parentNode[attr].toLowerCase().indexOf(value.toLowerCase()) !== -1){
            obj = ele.parentNode;
        }else{
            findEle(ele.parentNode,attr,value);
        }
    }
...
function findChildEle(ele,attr,value){

    var obj = null;

    findEle(ele,attr,value);

    //递归不能return，所以要在外面声明一个obj
...
//找到有特定属性的父级
function hasParent(ele, attr, value){
    var result = false;

    findEle(ele, attr, value);

    function findEle(ele, attr, value){
        if(ele && ele.parentNode && ele.parentNode.nodeType == 1){
            if(ele.parentNode[attr].toLowerCase().indexOf(value.toLowerCase()) !== -1){
                result = true;
            }else{
                findEle(ele.parentNode, attr,value);
...
function centerPopup(obj){
    ...
    if(!obj) {
        obj = document.querySelectorAll('.popup-center');
    }
    ...
    for(let pop of obj){
        if(!pop.classList.contains('hidden')){
            ...
            //获取到元素的高宽
            var popupH = pop.offsetHeight;
            var popupW = pop.offsetWidth;
